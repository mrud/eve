...
types the `output` field. This method does not change the target's sandbox: a root copy still
shares its parent's sandbox, and a declared subagent still uses its authored sandbox.
