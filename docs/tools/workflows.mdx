...
A workflow body may be an async generator. Yields report progress in both execution modes.
After processing a yield, eve advances the generator; use an awaited workflow operation when the
body needs to suspend.

| Operation       | Default execution                                                 | `execution: "background"`                                                  |
| --------------- | ----------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `yield value`   | Emits an `action.partial` snapshot for the pending tool call.     | Emits an `action.partial` snapshot without requesting a parent-agent turn. |
...
This background workflow publishes an intermediate progress snapshot, then suspends until its timer expires:
...
export default defineWorkflowTool({
  ...
  async *execute({ note, delay }) {
    "use workflow";
    yield { status: "preparing reminder" }; // Visible to session-stream clients; not sent to the model.
