...
background yields publish `action.partial` snapshots to session-stream clients without waking the
parent model or becoming task output. In a workflow body, keep values needed after a wait in local
variables; workflow replay reconstructs them across durable waits.
